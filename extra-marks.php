...
<?
$sqlExtramarks = $cn->selectdb("SELECT pdf_file from tbl_product where product_id=1");
if( $cn->numRows($sqlExtramarks) > 0 )
{
	$rowExtramarks = $cn->fetchAssoc($sqlExtramarks);
	extract($rowExtramarks);
