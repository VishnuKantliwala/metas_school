...
$sql = $cn->selectdb("SELECT * FROM tbl_download order by download_id desc");
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_download");												
						 //print_r($sql1);
while(empty($sql1)==0 AND $row1 = mysqli_fetch_array($sql1))
{ 
	$objSheet->getCell('A'.$i)->setValue($row1['download_id']);
	$objSheet->getCell('B'.$i)->setValue($row1['download_name']);
...
$multi_images = trim($allDataInSheet[$i]["F"]);
	...
	$query = "SELECT download_id FROM tbl_download WHERE download_id = ".$download_id;
	$sql = $con->selectdb($query);
	$recResult = mysqli_fetch_array($sql);
	$existid = $recResult["download_id"];
	
	if($existid>0) {
		//$msg = 'Record already exist. so updated it.... <div style="Padding:20px 0 0 0;"><a href="">Go Back to tutorial</a></div>';
		$insertTable= $con->selectdb("UPDATE tbl_download SET download_name = '".$download_name."', description = '".$description."',cat_id = '".$cat_id."',product_image = '".$product_image."',multi_images = '".$multi_images."' WHERE download_id = ".$download_id.";");
		//echo "UPDATE tbl_download SET product_title = '".$title."', product_imagelist = '".$imagelist."', product_desc = '".$desc."', price = '".$price."', cat_id=".$catid.", size = '".$size."' WHERE download_id = ".$id.";";
	
	} else {
		$insertTable= $con->selectdb("INSERT INTO tbl_download (download_id,download_name, description, cat_id,product_image,multi_images) VALUES ('".$download_id."','".$download_name."', '".$description."', '".$cat_id."','".$product_image."','".$multi_images."');");
                            ...
                            <div class="card-box">
                                ...
                                <?php
                                    ...
                                    for($i=0;$i<$cnt;$i++)
                                    {
                                        ...
                                        $sql=  $cn->selectdb("select * from tbl_download where download_id=$del_id");
                                        ...
                                        $con->selectdb("delete from `tbl_download` where download_id=".$del_id);
                                    }
                                        echo "<script>window.open('downloadView.php','_SELF')</script>";
                                    }
                                ?>
                                ...
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <a href="download.php" class="btn btn-success m-b-sm mt-2 mb-2">Add</a>
                                            <a href="sorting_download.php" class="btn btn-success m-b-sm mt-2 mb-2">Sort</a>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            ...
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap">
                                                ...
                                                ?>
                                                    <tr>
                                                        <td><input type="checkbox" name="chkbox[]" id="chkbox" class="chkbox"  value="<?echo $download_id?>"/></td>
                                                        <td><?php echo $download_name ?></td>
                                                        <td><? for($i=0; $i<$cnt;$i++) 
                                                                {
                                                                ?>
                                                                <?php echo $cn->getname('tbl_downloadcategory','cat_id','cat_name',$catID[$i]); ?>,
                                                                
                                                                <? } ?>
                                                        </td>
                                                        <td><a href='download_copy.php?id=<?php echo $download_id ?>&page=<?php  echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a></td>
                                                        <td><a href='download_up.php?download_id=<?php echo $download_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
                                                        <td><a href='delete_download_rec.php?tablename=tbl_download&primarykey=download_id&id=<?php echo $download_id ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
