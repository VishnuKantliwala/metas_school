...
include_once("delete_council_rec.php");
...
$sql = $cn->selectdb("SELECT * FROM tbl_council order by product_id desc");
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_council");												
						 //print_r($sql1);
while(empty($sql1)==0 AND $row1 = mysqli_fetch_array($sql1))
{ 
	$objSheet->getCell('A'.$i)->setValue($row1['product_id']);
						...
						|| ($_FILES["imageName"]["type"][$i] == "image/bmp"))
						  ...
						  if ($_FILES["imageName"]["error"][$i] > 0)
							...
							{	
									...
									move_uploaded_file($_FILES["imageName"]["tmp_name"][$i],"../council/big_img/" . $name4);
									
										
									make_thumb("../council/big_img/".$name4,"../council/".$name4,500,$extension);
...
$multi_images = trim($allDataInSheet[$i]["F"]);
	...
	$query = "SELECT product_id FROM tbl_council WHERE product_id = ".$product_id;
	$sql = $con->selectdb($query);
	$recResult = mysqli_fetch_array($sql);
	$existid = $recResult["product_id"];
	
	if($existid>0) {
		//$msg = 'Record already exist. so updated it.... <div style="Padding:20px 0 0 0;"><a href="">Go Back to tutorial</a></div>';
		$insertTable= $con->selectdb("UPDATE tbl_council SET product_name = '".$product_name."', description = '".$description."',cat_id = '".$cat_id."',product_image = '".$product_image."',multi_images = '".$multi_images."' WHERE product_id = ".$product_id.";");
		//echo "UPDATE tbl_council SET product_title = '".$title."', product_imagelist = '".$imagelist."', product_desc = '".$desc."', price = '".$price."', cat_id=".$catid.", size = '".$size."' WHERE product_id = ".$id.";";
	
	} else {
		$insertTable= $con->selectdb("INSERT INTO tbl_council (product_id,product_name, description, cat_id,product_image,multi_images) VALUES ('".$product_id."','".$product_name."', '".$description."', '".$cat_id."','".$product_image."','".$multi_images."');");
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">  Council Circulars (CISCE)  View</h4>
                                <?php
                                    ...
                                    for($i=0;$i<$cnt;$i++)
                                    {
                                        ...
                                        //$con->selectdb("delete from `tbl_council` where product_id=".$del_id);
                                    }
                                        echo "<script>window.open('councilView.php','_SELF')</script>";
                                    }
                                ?>
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <a href="council.php" class="btn btn-success m-b-sm mt-2 mb-2">Add</a>
                                    ...
                                    <table id="datatable" class="table table-bordered dt-responsive nowrap">
										...
										?>
                                            <tr>
												...
												<td><a href='council_up.php?product_id=<?php echo $product_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
												<td><a href='delete_council_rec.php?tablename=tbl_council&primarykey=product_id&id=<?php echo $product_id ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
                                            </tr>
											<? } } ?>
