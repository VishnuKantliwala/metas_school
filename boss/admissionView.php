...
$sql = $cn->selectdb("SELECT * FROM tbl_admission order by blog_id desc");
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_admission");												
						 //print_r($sql1);
						...
						|| ($_FILES["imageName"]["type"][$i] == "image/bmp"))
						  ...
						  if ($_FILES["imageName"]["error"][$i] > 0)
							...
							{	
									...
									move_uploaded_file($_FILES["imageName"]["tmp_name"][$i],"../admission/big_img/" . $name4);
									
										
									make_thumb("../admission/big_img/".$name4,"../admission/".$name4,500,$extension);
...
$multi_images = trim($allDataInSheet[$i]["F"]);
	...
	$query = "SELECT blog_id FROM tbl_admission WHERE blog_id = ".$blog_id;
	...
	if($existid>0) {
		...
		$insertTable= $con->selectdb("UPDATE tbl_admission SET blog_name = '".$blog_name."', description = '".$description."',cat_id = '".$cat_id."',blog_image = '".$blog_image."',multi_images = '".$multi_images."' WHERE blog_id = ".$blog_id.";");
		//echo "UPDATE tbl_admission SET product_title = '".$title."', blog_imagelist = '".$imagelist."', product_desc = '".$desc."', price = '".$price."', cat_id=".$catid.", size = '".$size."' WHERE blog_id = ".$id.";";
	
	} else {
		$insertTable= $con->selectdb("INSERT INTO tbl_admission (blog_id,blog_name, description, cat_id,blog_image,multi_images) VALUES ('".$blog_id."','".$blog_name."', '".$description."', '".$cat_id."','".$blog_image."','".$multi_images."');");
...
function deleteblog($id){
    ...
    $cn=new connect();
    $cn->connectdb();
    $sql=  $cn->selectdb("select * from tbl_admission where blog_id=$id");
	while($row = mysqli_fetch_row($sql))
	{
		...
		foreach($image_list as $rowF)
		{
			...
			$new_image_list = '';
			@unlink('../admissionF/big_img/'.$rowF);
			@unlink('../admissionF/'.$rowF);
		}
	...
	$cn->selectdb("delete from tbl_admission where blog_id=$id");
    ...
    <div id="wrapper">
        ...
        <div class="navbar-custom">
            ...
            <ul class="list-unstyled topnav-menu topnav-menu-left m-0">
                ...
                <li>
                    <h4 class="page-title-main">Admission</h4>
                </li>
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Admission View</h4>
                                <?php
                                    ...
                                    for($i=0;$i<$cnt;$i++)
                                    {
                                        ...
                                        echo "<script>window.open('admissionView.php','_SELF')</script>";
                                    }
                                ?>
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <a href="admission.php" class="btn btn-success m-b-sm mt-2 mb-2">Add</a>
                                            <!-- <a href="sorting_admission.php" class="btn btn-success m-b-sm mt-2 mb-2">Sort</a> -->
                                    ...
                                    <table id="datatable" class="table table-bordered dt-responsive nowrap">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
												    <a href='admission_copy.php?id=<?php echo $blog_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a>
                                                </td>
                                                <td><a href='admission_up.php?blog_id=<?php echo $blog_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
												<td><a href='delete_admission_rec.php?tablename=tbl_admission&primarykey=blog_id&id=<?php echo $blog_id ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
