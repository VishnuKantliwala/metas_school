...
else
{
    ...
    $result = $cn->selectdb("SELECT cat_id,cat_parent_id,cat_name,cat_description,cat_image FROM tbl_downloadcategory where cat_parent_id=".$parentid." order by cat_id desc");
...
{   
    ...
    $var="";
    //write your query      
    $sql="select * from tbl_downloadcategory";
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_downloadcategory");												
						 //print_r($sql1);
...
$architect_id = trim($allDataInSheet[$i]["E"]);
	...
	$query = "SELECT cat_id FROM tbl_downloadcategory WHERE cat_id = ".$cat_id;
	...
	if($existid>0) {
		...
		$insertTable= $con->selectdb("UPDATE tbl_downloadcategory SET cat_parent_id = '".$cat_parent_id."', cat_name = '".$cat_name."',cat_image = '".$cat_image."',architect_id = '".$architect_id."' WHERE cat_id = ".$cat_id.";");
		...
		$insertTable= $con->selectdb("INSERT INTO tbl_downloadcategory (cat_id,cat_parent_id, cat_name, cat_image,architect_id) VALUES ('".$cat_id."','".$cat_parent_id."', '".$cat_name."', '".$cat_image."','".$architect_id."');");
                            ...
                            <div class="card-box">
                                ...
                                <?php
                                    if(isset($_POST['delete']))
                                    {
                                        ...
                                        echo "<script>window.open('downloadCatView.php','_SELF')</script>";
                                    }
                                ?>
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row mt-2 mb-2">
                                        <div class="col-lg-12">
                                            <a href="downloadCategory.php?parentid=<?php echo $parentid; ?>" class="btn btn-success m-b-sm">Add Category</a>
								            <a href="sorting_download_category.php" class="btn btn-success m-b-sm">Sorting</a>
                                            ...
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap">
                                                ...
                                                if (mysqli_num_rows($result) > 0) 
                                                {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><a href="downloadCatView.php?parentid=<?php echo $cat_id; ?>"><?php echo $cat_name; ?></a></td>
                                                        <td><?php if($cat_image!=''){?><img src='../downloadcategory/<?php echo $cat_image; ?>' height="100" width="100"><?php } else { echo "No Image"; }?></td>
                                                        <td><a href='downloadCategory_copy.php?id=<?php echo $cat_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a></td>
                                                        <td><a href='downloadcategory_up.php?category_id=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
                                                        <td><a href='deleteDownloadCat.php?catId=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
