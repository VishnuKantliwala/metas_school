...
include_once("delete_submenu.php");
...
$sql = $cn->selectdb("SELECT * FROM tbl_submenu order by submenu_id desc");
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_submenu");												
						 //print_r($sql1);
while(empty($sql1)==0 AND $row1 = mysqli_fetch_array($sql1))
{ 
	$objSheet->getCell('A'.$i)->setValue($row1['submenu_id']);
	$objSheet->getCell('B'.$i)->setValue($row1['submenu_name']);
	$objSheet->getCell('C'.$i)->setValue($row1['description']);
	$objSheet->getCell('D'.$i)->setValue($row1['cat_id']);
	$objSheet->getCell('E'.$i)->setValue($row1['submenu_image']);
...
<?
//start coding for uploading submenus single images for excel entries
						...
						|| ($_FILES["imageName"]["type"][$i] == "image/bmp"))
						  ...
						  if ($_FILES["imageName"]["error"][$i] > 0)
							...
							{	
									...
									move_uploaded_file($_FILES["imageName"]["tmp_name"][$i],"../submenu/big_img/" . $name4);
									
										
									make_thumb("../submenu/big_img/".$name4,"../submenu/".$name4,500,$extension);
...
//end coding for uploading submenus multiple images for excel entries
?>
...
while($i<=$arrayCount)
{
$submenu_id = trim($allDataInSheet[$i]["A"]);

$submenu_name = trim($allDataInSheet[$i]["B"]);
$description = trim($allDataInSheet[$i]["C"]);
$cat_id = trim($allDataInSheet[$i]["D"]);
$submenu_image = trim($allDataInSheet[$i]["E"]);
$multi_images = trim($allDataInSheet[$i]["F"]);


	$query = "SELECT submenu_id FROM tbl_submenu WHERE submenu_id = ".$submenu_id;
	$sql = $con->selectdb($query);
	$recResult = mysqli_fetch_array($sql);
	$existid = $recResult["submenu_id"];
	
	if($existid>0) {
		//$msg = 'Record already exist. so updated it.... <div style="Padding:20px 0 0 0;"><a href="">Go Back to tutorial</a></div>';
		$insertTable= $con->selectdb("UPDATE tbl_submenu SET submenu_name = '".$submenu_name."', description = '".$description."',cat_id = '".$cat_id."',submenu_image = '".$submenu_image."',multi_images = '".$multi_images."' WHERE submenu_id = ".$submenu_id.";");
		//echo "UPDATE tbl_submenu SET submenu_title = '".$title."', submenu_imagelist = '".$imagelist."', submenu_desc = '".$desc."', price = '".$price."', cat_id=".$catid.", size = '".$size."' WHERE submenu_id = ".$id.";";
	
	} else {
		$insertTable= $con->selectdb("INSERT INTO tbl_submenu (submenu_id,submenu_name, description, cat_id,submenu_image,multi_images) VALUES ('".$submenu_id."','".$submenu_name."', '".$description."', '".$cat_id."','".$submenu_image."','".$multi_images."');");
                            ...
                            <div class="card-box">
                                ...
                                <?php
                                    ...
                                    for($i=0;$i<$cnt;$i++)
                                    {
                                        ...
                                        deleteSubmenu($del_id);
                                    }
                                        echo "<script>window.open('submenuView.php','_SELF')</script>";
                                    }
                                ?>
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <a href="submenu.php" class="btn btn-success m-b-sm mt-2 mb-2">Add</a>
                                            <a href="sorting_submenu.php" class="btn btn-success m-b-sm mt-2 mb-2">Sort</a>
                                            ...
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap">
                                                ...
                                                if (mysqli_num_rows($sql) > 0) 
                                                {
                                                    ...
                                                    while($row = mysqli_fetch_assoc($sql)) 
                                                    {
                                                        ...
                                                        ?>
                                                    <tr>
                                                        <td><input type="checkbox" name="chkbox[]" id="chkbox" class="chkbox"  value="<?echo $submenu_id?>"/></td>
                                                        <td><?php echo $submenu_name ?></td>
                                                        <td>
                                                            <? for($i=0; $i<$cnt;$i++) 
                                                            {
                                                                echo $cn->getname('tbl_dmenu','menu_id','menu_name',$menuID[$i]); ?>,
                                                            <? } ?>
                                                        </td>
                                                        <td>
                                                            <a href='submenu_copy.php?id=<?php echo $submenu_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a>
                                                        </td>
                                                        <td>
                                                            <a href='submenu_up.php?submenu_id=<?php echo $submenu_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a>
                                                        </td>
                                                        <td>
                                                            <a href='delete_submenu.php?tablename=tbl_submenu&primarykey=submenu_id&id=<?php echo $submenu_id ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a>
                                                        </td>
