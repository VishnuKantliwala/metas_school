...
$sql = $cn->selectdb("SELECT * FROM tbl_video order by video_id desc");
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_video");												
						 //print_r($sql1);
while(empty($sql1)==0 AND $row1 = mysqli_fetch_array($sql1))
{ 
	$objSheet->getCell('A'.$i)->setValue($row1['video_id']);
...
while($i<=$arrayCount)
{
$video_id = trim($allDataInSheet[$i]["A"]);
...
$multi_images = trim($allDataInSheet[$i]["F"]);
	...
	$query = "SELECT video_id FROM tbl_video WHERE video_id = ".$video_id;
	$sql = $con->selectdb($query);
	$recResult = mysqli_fetch_array($sql);
	$existid = $recResult["video_id"];
	
	if($existid>0) {
		//$msg = 'Record already exist. so updated it.... <div style="Padding:20px 0 0 0;"><a href="">Go Back to tutorial</a></div>';
		$insertTable= $con->selectdb("UPDATE tbl_video SET product_name = '".$product_name."', description = '".$description."',cat_id = '".$cat_id."',product_image = '".$product_image."',multi_images = '".$multi_images."' WHERE video_id = ".$video_id.";");
		//echo "UPDATE tbl_video SET product_title = '".$title."', product_imagelist = '".$imagelist."', product_desc = '".$desc."', price = '".$price."', cat_id=".$catid.", size = '".$size."' WHERE video_id = ".$id.";";
	
	} else {
		$insertTable= $con->selectdb("INSERT INTO tbl_video (video_id,product_name, description, cat_id,product_image,multi_images) VALUES ('".$video_id."','".$product_name."', '".$description."', '".$cat_id."','".$product_image."','".$multi_images."');");
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Video View</h4>
                                <?php
                                    ...
                                    for($i=0;$i<$cnt;$i++)
                                    {
                                        ...
                                        $con->selectdb("delete from `tbl_video` where video_id=".$del_id);
                                    }
                                        echo "<script>window.open('videoView.php','_SELF')</script>";
                                    }
                                ?>
                                ...
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <a href="video.php" class="btn btn-success m-b-sm mt-2 mb-2">Add</a>
                                            <a href="sorting_video.php" class="btn btn-success m-b-sm mt-2 mb-2">Sort</a>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <div class="row">
                                                                    <div class="col-12">
                                                                        <? for($i=0; $i<$cnt;$i++){?>
                                                                            <?php echo $cn->getname('tbl_video_category','cat_id','cat_name',$catID[$i]); ?>,
                                                                        <?}?>
                                                                    </div>
                                                                </div>
                                                            </td>
