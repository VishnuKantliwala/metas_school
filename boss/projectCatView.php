...
$result = $cn->selectdb("SELECT cat_id,cat_parent_id,cat_name,cat_description,cat_image FROM tbl_project_category where cat_parent_id=".$parentid." order by cat_id desc");
...
{   
    ...
    $var="";
    //write your query      
    $sql="select * from tbl_project_category";
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_project_category");												
						 //print_r($sql1);
						...
						|| ($_FILES["imageName"]["type"][$i] == "image/bmp"))
						  ...
						  if ($_FILES["imageName"]["error"][$i] > 0)
							...
							{	
									...
									move_uploaded_file($_FILES["imageName"]["tmp_name"][$i],"../projectcategory/big_img/" . $name4);
									
										
									make_thumb("../projectcategory/big_img/".$name4,"../projectcategory/".$name4,500,$extension);
...
$architect_id = trim($allDataInSheet[$i]["E"]);
	...
	$query = "SELECT cat_id FROM tbl_project_category WHERE cat_id = ".$cat_id;
	...
	if($existid>0) {
		...
		$insertTable= $con->selectdb("UPDATE tbl_project_category SET cat_parent_id = '".$cat_parent_id."', cat_name = '".$cat_name."',cat_image = '".$cat_image."',architect_id = '".$architect_id."' WHERE cat_id = ".$cat_id.";");
		...
		$insertTable= $con->selectdb("INSERT INTO tbl_project_category (cat_id,cat_parent_id, cat_name, cat_image,architect_id) VALUES ('".$cat_id."','".$cat_parent_id."', '".$cat_name."', '".$cat_image."','".$architect_id."');");
...
function delmultirec($id)
{
	...
	$sql = "SELECT cat_id,project_id,project_image,multi_images
	        FROM tbl_project
	...
	$result = dbQuery($sql);
		...
		while ($row = dbFetchAssoc($result)) {
			...
			//echo $row[0];die;
				...
				if($cntCatId>1)
				{
				  $sqlupdate =mysqli_query("UPDATE tbl_project SET cat_id = replace(cat_id, '".$categories[$i].",', '') WHERE FIND_IN_SET('".$categories[$i]."',cat_id)>0");

				}
				elseif($cntCatId== 1)
				{
					$image_list = explode(',',$row['project_image']);
					...
					foreach($image_list1 as $rowF1)
					{
						//print_r($image_list);die;
						$new_image_list1 = '';
						unlink('../projectF/big_img/'.$rowF1);
						unlink('../projectF/'.$rowF1);
					}
					// end of multiple images
					
					// delete the products
					$sqlD = "DELETE FROM tbl_project
    ...
    $sql = "DELETE FROM tbl_project_category 
            WHERE cat_id IN (" . implode(',', $categories) . ")";
    dbQuery($sql);
...
function deleteImage()
{
	...
	$sql = "UPDATE tbl_project_category
			SET cat_image = ''
			WHERE cat_id = $catId";
	dbQuery($sql);        
...
function _deleteImage($id)
{
    ...
    if (dbNumRows($result)) {
        while ($row = dbFetchAssoc($result)) {
			...
			$deleted = @unlink("../projectcategory/". $row['cat_image']);	
			@unlink("../projectcategory/big_img/". $row['cat_image']);	
        }	
        $deleted = true;
    }
    ...
    <div id="wrapper">
        ...
        <div class="navbar-custom">
            ...
            <ul class="list-unstyled topnav-menu topnav-menu-left m-0">
                ...
                <li>
                    <h4 class="page-title-main">Project Category</h4>
                </li>
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Project Category View</h4>
                                <?php
                                    if(isset($_POST['delete']))
                                    {
                                        ...
                                        for($i=0;$i<$cnt;$i++)
                                        {
                                            ...
                                            delmultirec($del_id);
                                            // $sql=  $cn->selectdb("select * from tbl_project_category where cat_id=$del_id");
                                            // while($row = mysqli_fetch_assoc($sql))
                                            // {
                                            //     //image
                                            //     @unlink('../projectcategory/big_img/'.$row['cat_image']);
                                            //     @unlink('../projectcategory/'.$row['cat_image']);
                                                
                                            // }
                                            
                                            //echo "<script>alert('".$del_id."');</script>";
                                            //$query="delete from product where product_id=".$del_id;
                                            // $con->selectdb("delete from `tbl_project_category` where cat_id=".$del_id);
                                        }
                                        echo "<script>window.open('projectCatView.php','_SELF')</script>";
                                    }
                                ?>
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row mt-2 mb-2">
                                        <div class="col-lg-12">
                                            <a href="projectCategory.php?parentid=<?php echo $parentid; ?>" class="btn btn-success m-b-sm">Add Category</a>
								            <a href="sorting_project_category.php" class="btn btn-success m-b-sm">Sorting</a>
                                            ...
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap">
                                                ...
                                                if (mysqli_num_rows($result) > 0) 
                                                {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><a href="projectCatView.php?parentid=<?php echo $cat_id; ?>"><?php echo $cat_name; ?></a></td>
                                                        <td><?php if($cat_image!=''){?><img src='../projectcategory/<?php echo $cat_image; ?>' height="100" width="100"><?php } else { echo "No Image"; }?></td>
                                                        <td><a href='projectCategory_copy.php?id=<?php echo $cat_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a></td>
                                                        <td><a href='projectcategory_up.php?category_id=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
                                                        <td><a href='deleteProjectCat.php?catId=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
