...
$sql = $cn->selectdb("SELECT * FROM tbl_magazine order by magazine_id desc");
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_magazine");												
						 //print_r($sql1);
while(empty($sql1)==0 AND $row1 = mysqli_fetch_array($sql1))
{ 
	$objSheet->getCell('A'.$i)->setValue($row1['magazine_id']);
	$objSheet->getCell('B'.$i)->setValue($row1['magazine_name']);
	$objSheet->getCell('C'.$i)->setValue($row1['description']);
	$objSheet->getCell('D'.$i)->setValue($row1['cat_id']);
	$objSheet->getCell('E'.$i)->setValue($row1['magazine_image']);
...
<?
//start coding for uploading magazines single images for excel entries
						...
						|| ($_FILES["imageName"]["type"][$i] == "image/bmp"))
						  ...
						  if ($_FILES["imageName"]["error"][$i] > 0)
							...
							{	
									...
									move_uploaded_file($_FILES["imageName"]["tmp_name"][$i],"../magazine/big_img/" . $name4);
									
										
									make_thumb("../magazine/big_img/".$name4,"../magazine/".$name4,500,$extension);
...
//end coding for uploading magazines multiple images for excel entries
?>
...
while($i<=$arrayCount)
{
$magazine_id = trim($allDataInSheet[$i]["A"]);

$magazine_name = trim($allDataInSheet[$i]["B"]);
$description = trim($allDataInSheet[$i]["C"]);
$cat_id = trim($allDataInSheet[$i]["D"]);
$magazine_image = trim($allDataInSheet[$i]["E"]);
$multi_images = trim($allDataInSheet[$i]["F"]);


	$query = "SELECT magazine_id FROM tbl_magazine WHERE magazine_id = ".$magazine_id;
	$sql = $con->selectdb($query);
	$recResult = mysqli_fetch_array($sql);
	$existid = $recResult["magazine_id"];
	
	if($existid>0) {
		//$msg = 'Record already exist. so updated it.... <div style="Padding:20px 0 0 0;"><a href="">Go Back to tutorial</a></div>';
		$insertTable= $con->selectdb("UPDATE tbl_magazine SET magazine_name = '".$magazine_name."', description = '".$description."',cat_id = '".$cat_id."',magazine_image = '".$magazine_image."',multi_images = '".$multi_images."' WHERE magazine_id = ".$magazine_id.";");
		//echo "UPDATE tbl_magazine SET magazine_title = '".$title."', magazine_imagelist = '".$imagelist."', magazine_desc = '".$desc."', price = '".$price."', cat_id=".$catid.", size = '".$size."' WHERE magazine_id = ".$id.";";
	
	} else {
		$insertTable= $con->selectdb("INSERT INTO tbl_magazine (magazine_id,magazine_name, description, cat_id,magazine_image,multi_images) VALUES ('".$magazine_id."','".$magazine_name."', '".$description."', '".$cat_id."','".$magazine_image."','".$multi_images."');");
    ...
    <div id="wrapper">
        ...
        <div class="navbar-custom">
            ...
            <ul class="list-unstyled topnav-menu topnav-menu-left m-0">
                ...
                <li>
                    <h4 class="page-title-main">Magazine</h4>
                </li>
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Magazine View</h4>
                                <?php
                                    ...
                                    for($i=0;$i<$cnt;$i++)
                                    {
                                        ...
                                        deleteMagazine($del_id);
                                        //echo "<script>alert('".$del_id."');</script>";
                                        //$query="delete from magazine where magazine_id=".$del_id;
                                        //$con->selectdb("delete from `tbl_magazine` where magazine_id=".$del_id);
                                    }
                                        echo "<script>window.open('magazineView.php','_SELF')</script>";
                                    }
                                ?>
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <!-- <a href="magazine.php" class="btn btn-success m-b-sm mt-2 mb-2">Add</a> -->
                                            <!-- <a href="sorting_magazine.php" class="btn btn-success m-b-sm mt-2 mb-2">Sort</a> -->
                                    ...
                                    <table id="datatable" class="table table-bordered dt-responsive nowrap">
										...
										?>
                                            <tr>
                                                <!-- <td><input type="checkbox" name="chkbox[]" id="chkbox" class="chkbox"  value="<?echo $magazine_id?>"/></td> -->
												<td><?php echo $magazine_name ?></td>
												<!-- <td><? for($i=0; $i<$cnt;$i++) 
														{
                                                ...
                                                <!-- <td>
												    <a href='magazine_copy.php?id=<?php echo $magazine_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a>
                                                </td> -->
												<td><a href='magazine_up.php?magazine_id=<?php echo $magazine_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
												<!-- <td><a href='delete_rec.php?tablename=tbl_magazine&primarykey=magazine_id&id=<?php echo $magazine_id ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td> -->
                                            </tr>
											<? } } ?>
