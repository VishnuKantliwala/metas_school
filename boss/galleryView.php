...
$sql = $cn->selectdb("SELECT * FROM tbl_gallery order by gallery_id desc");
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_gallery");												
						 //print_r($sql1);
while(empty($sql1)==0 AND $row1 = mysqli_fetch_array($sql1))
{ 
	$objSheet->getCell('A'.$i)->setValue($row1['gallery_id']);
	$objSheet->getCell('B'.$i)->setValue($row1['product_name']);
	$objSheet->getCell('C'.$i)->setValue($row1['description']);
	$objSheet->getCell('D'.$i)->setValue($row1['cat_id']);
	$objSheet->getCell('E'.$i)->setValue($row1['product_image']);
						...
						|| ($_FILES["imageName"]["type"][$i] == "image/bmp"))
						  ...
						  if ($_FILES["imageName"]["error"][$i] > 0)
							...
							{	
									...
									move_uploaded_file($_FILES["imageName"]["tmp_name"][$i],"../product/big_img/" . $name4);
									
										
									make_thumb("../product/big_img/".$name4,"../product/".$name4,500,$extension);
...
while($i<=$arrayCount)
{
$gallery_id = trim($allDataInSheet[$i]["A"]);
...
$multi_images = trim($allDataInSheet[$i]["F"]);
	...
	$query = "SELECT gallery_id FROM tbl_gallery WHERE gallery_id = ".$gallery_id;
	$sql = $con->selectdb($query);
	$recResult = mysqli_fetch_array($sql);
	$existid = $recResult["gallery_id"];
	
	if($existid>0) {
		//$msg = 'Record already exist. so updated it.... <div style="Padding:20px 0 0 0;"><a href="">Go Back to tutorial</a></div>';
		$insertTable= $con->selectdb("UPDATE tbl_gallery SET product_name = '".$product_name."', description = '".$description."',cat_id = '".$cat_id."',product_image = '".$product_image."',multi_images = '".$multi_images."' WHERE gallery_id = ".$gallery_id.";");
		//echo "UPDATE tbl_gallery SET product_title = '".$title."', product_imagelist = '".$imagelist."', product_desc = '".$desc."', price = '".$price."', cat_id=".$catid.", size = '".$size."' WHERE gallery_id = ".$id.";";
	
	} else {
		$insertTable= $con->selectdb("INSERT INTO tbl_gallery (gallery_id,product_name, description, cat_id,product_image,multi_images) VALUES ('".$gallery_id."','".$product_name."', '".$description."', '".$cat_id."','".$product_image."','".$multi_images."');");
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Gallery View</h4>
                                <?php
                                    ...
                                    for($i=0;$i<$cnt;$i++)
                                    {
                                        ...
                                        $sql=  $cn->selectdb("select * from tbl_gallery where gallery_id=$del_id");
                                        ...
                                        $con->selectdb("delete from `tbl_gallery` where gallery_id=".$del_id);
                                    }
                                        echo "<script>window.open('galleryView.php','_SELF')</script>";
                                    }
                                ?>
                                ...
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <a href="gallery.php" class="btn btn-success m-b-sm mt-2 mb-2">Add</a>
                                            <a href="sorting_gallery.php" class="btn btn-success m-b-sm mt-2 mb-2">Sort</a>
                                        ...
                                        <div class="col-sm-12">
                                            ...
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap">
                                                ...
                                                <tbody>
                                                    ...
                                                    ?>
                                                    <tr>
                                                        <td><input type="checkbox" name="chkbox[]" id="chkbox" class="chkbox"  value="<?echo $gallery_id?>"/></td>
                                                        <td><?php echo $gallery_name ?></td>
                                                        <td>
                                                            <div class="row">
                                                                <div class="col-12">
                                                                    <?php  for($i=0; $i<$cnt;$i++){ ?>
                                                                        <?php echo $cn->getname('tbl_gallery_category','cat_id','cat_name',$catID[$i]); ?>,
                                                                    <?php  } ?>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td><a href='gallery_copy.php?id=<?php echo $gallery_id ?>&page=<?php  echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a></td>
                                                        <td><a href='gallery_up.php?gallery_id=<?php echo $gallery_id ?>&page=<?php  echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
                                                        <td><a href='delete_gallery_rec.php?tablename=tbl_gallery&primarykey=gallery_id&id=<?php echo $gallery_id ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
