...
else
{
    ...
    $result = $cn->selectdb("SELECT cat_id,cat_parent_id,cat_name,cat_description,cat_image FROM tbl_service_category where cat_parent_id=".$parentid." order by cat_id desc");
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_service_category");												
						 //print_r($sql1);
...
$architect_id = trim($allDataInSheet[$i]["E"]);
	...
	$query = "SELECT cat_id FROM tbl_service_category WHERE cat_id = ".$cat_id;
	...
	if($existid>0) {
		...
		$insertTable= $con->selectdb("UPDATE tbl_service_category SET cat_parent_id = '".$cat_parent_id."', cat_name = '".$cat_name."',cat_image = '".$cat_image."',architect_id = '".$architect_id."' WHERE cat_id = ".$cat_id.";");
		...
		$insertTable= $con->selectdb("INSERT INTO tbl_service_category (cat_id,cat_parent_id, cat_name, cat_image,architect_id) VALUES ('".$cat_id."','".$cat_parent_id."', '".$cat_name."', '".$cat_image."','".$architect_id."');");
...
function delmultirec($id)
{
	...
	$sql = "SELECT cat_id,service_id,image_name
	        FROM tbl_service
	...
	$result = dbQuery($sql);
		...
		while ($row = dbFetchAssoc($result)) {
			...
			//echo $row[0];die;
				...
				if($cntCatId>1)
				{
				  $sqlupdate =mysqli_query("UPDATE tbl_service SET cat_id = replace(cat_id, '".$categories[$i].",', '') WHERE FIND_IN_SET('".$categories[$i]."',cat_id)>0");
    ...
    $sql = "DELETE FROM tbl_service_category 
            WHERE cat_id IN (" . implode(',', $categories) . ")";
    dbQuery($sql);
...
function deleteImage()
{
	...
	$sql = "UPDATE tbl_service_category
			SET cat_image = ''
			WHERE cat_id = $catId";
	dbQuery($sql);        
...
function _deleteImage($id)
{
    ...
    if (dbNumRows($result)) {
        while ($row = dbFetchAssoc($result)) {
			...
			$deleted = @unlink("../servicecategory/". $row['cat_image']);	
			@unlink("../servicecategory/big_img/". $row['cat_image']);	
        }	
        $deleted = true;
    }
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Service Category View</h4>
                                <?php
                                    if(isset($_POST['delete']))
                                    {
                                        ...
                                        echo "<script>window.open('serviceCatView.php','_SELF')</script>";
                                    }
                                ?>
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row mt-2 mb-2">
                                        <div class="col-lg-12">
                                            <a href="serviceCategory.php?parentid=<?php echo $parentid; ?>" class="btn btn-success m-b-sm">Add Category</a>
								            <a href="sorting_service_category.php" class="btn btn-success m-b-sm">Sorting</a>
                                            ...
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap">
                                                ...
                                                if (mysqli_num_rows($result) > 0) 
                                                {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><a href="serviceCatView.php?parentid=<?php echo $cat_id; ?>"><?php echo $cat_name; ?></a></td>
                                                        <td><?php if($cat_image!=''){?><img src='../servicecategory/<?php echo $cat_image; ?>' height="100" width="100"><?php } else { echo "No Image"; }?></td>
                                                        <td><a href='serviceCategory_copy.php?id=<?php echo $cat_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a></td>
                                                        <td><a href='servicecategory_up.php?category_id=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
                                                        <td><a href='deleteServiceCat.php?catId=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
