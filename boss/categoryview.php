...
else
{
    ...
    $result = $cn->selectdb("SELECT cat_id,cat_parent_id,cat_name,cat_image FROM tbl_category where cat_parent_id=".$parentid." order by cat_id desc");
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_category");												
						 //print_r($sql1);
...
$architect_id = trim($allDataInSheet[$i]["E"]);
	...
	if($existid>0) {
		...
		$insertTable= $con->selectdb("UPDATE tbl_category SET cat_parent_id = '".$cat_parent_id."', cat_name = '".$cat_name."',cat_image = '".$cat_image."',architect_id = '".$architect_id."' WHERE cat_id = ".$cat_id.";");
		...
		$insertTable= $con->selectdb("INSERT INTO tbl_category (cat_id,cat_parent_id, cat_name, cat_image,architect_id) VALUES ('".$cat_id."','".$cat_parent_id."', '".$cat_name."', '".$cat_image."','".$architect_id."');");
                            ...
                            <div class="card-box">
                                ...
                                <?php
									...
									for($i=0;$i<$cnt;$i++)
									{
										...
										echo "<script>window.open('categoryview.php','_SELF')</script>";
									}
								?>
									...
									<tfoot>
										...
										?>
                                            <tr>
												...
												<td><a href="categoryview.php?parentid=<?php echo $cat_id; ?>"><?php echo $cat_name; ?></a></td>
												<td><?php if($cat_image!=''){?><img src='../category/<?php echo $cat_image; ?>' height="100" width="100"><?php } else { echo "No Image"; }?></td>
                                                
												 <td>
												<a href='category_copy.php?id=<?php echo $cat_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a>
												
												</td>
												
												<td><a href='category_up.php?category_id=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
												<td><a href='deletecat.php?catId=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
