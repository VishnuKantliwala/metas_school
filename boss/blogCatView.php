...
else
{
    ...
    $result = $cn->selectdb("SELECT cat_id,cat_parent_id,cat_name,cat_description,cat_image FROM tbl_blogcategory where cat_parent_id=".$parentid." order by cat_id desc");
...
{   
    ...
    $var="";
    //write your query      
    $sql="select * from tbl_blogcategory";
...
$architect_id = trim($allDataInSheet[$i]["E"]);
	...
	$query = "SELECT cat_id FROM tbl_blogcategory WHERE cat_id = ".$cat_id;
	...
	if($existid>0) {
		...
		$insertTable= $con->selectdb("UPDATE tbl_blogcategory SET cat_parent_id = '".$cat_parent_id."', cat_name = '".$cat_name."',cat_image = '".$cat_image."',architect_id = '".$architect_id."' WHERE cat_id = ".$cat_id.";");
		...
		$insertTable= $con->selectdb("INSERT INTO tbl_blogcategory (cat_id,cat_parent_id, cat_name, cat_image,architect_id) VALUES ('".$cat_id."','".$cat_parent_id."', '".$cat_name."', '".$cat_image."','".$architect_id."');");
...
function delmultiblogcat($id)
{
	...
	$sql = "SELECT cat_id,blog_id,blog_image,multi_images
	        FROM tbl_blog
	...
	$result = dbQuery($sql);
		...
		while ($row = dbFetchAssoc($result)) {
			...
			//echo $row[0];die;
				...
				if($cntCatId>1)
				{
				  $sqlupdate =mysqli_query("UPDATE tbl_blog SET cat_id = replace(cat_id, '".$categories[$i].",', '') WHERE FIND_IN_SET('".$categories[$i]."',cat_id)>0");

				}
				elseif($cntCatId== 1)
				{
					$image_list = explode(',',$row['blog_image']);
    ...
    $sql = "DELETE FROM tbl_blogcategory 
            WHERE cat_id IN (" . implode(',', $categories) . ")";
    dbQuery($sql);
...
function _deleteImage($id)
{
    ...
    if (dbNumRows($result)) {
        while ($row = dbFetchAssoc($result)) {
			...
			$deleted = @unlink("../blogcategory/". $row['cat_image']);	
			@unlink("../blogcategory/big_img/". $row['cat_image']);	
        }	
        $deleted = true;
    }
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Blog Category View</h4>
                                <?php
									...
									for($i=0;$i<$cnt;$i++)
									{
										...
										echo "<script>window.open('blogCatView.php','_SELF')</script>";
									}
								?>
                                ...
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <a href="blogcategory.php?parentid=<?php echo $parentid; ?>" class="btn btn-success m-b-sm mt-2 mb-2">Add Category</a>
									...
									<tfoot>
										...
										if (mysqli_num_rows($result) > 0) 
										{
                                            ...
                                            <tr>
												...
												<td><a href="blogCatView.php?parentid=<?php echo $cat_id; ?>"><?php echo $cat_name; ?></a></td>
												<td><?php if($cat_image!=''){?><img src='../blogcategory/<?php echo $cat_image; ?>' height="100" width="100"><?php } else { echo "No Image"; }?></td>
                                                <td><a href='blogcategory_up.php?category_id=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
												<td><a href='deleteblog_cat.php?catId=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
