...
else
{
    ...
    $result = $cn->selectdb("SELECT cat_id,cat_parent_id,cat_name,cat_description,cat_image FROM tbl_gallery_category where cat_parent_id=".$parentid." order by cat_id desc");
...
{   
    ...
    $var="";
    //write your query      
    $sql="select * from tbl_gallery_category";
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_gallery_category");												
						 //print_r($sql1);
...
$architect_id = trim($allDataInSheet[$i]["E"]);
	...
	$query = "SELECT cat_id FROM tbl_gallery_category WHERE cat_id = ".$cat_id;
	...
	if($existid>0) {
		...
		$insertTable= $con->selectdb("UPDATE tbl_gallery_category SET cat_parent_id = '".$cat_parent_id."', cat_name = '".$cat_name."',cat_image = '".$cat_image."',architect_id = '".$architect_id."' WHERE cat_id = ".$cat_id.";");
		...
		$insertTable= $con->selectdb("INSERT INTO tbl_gallery_category (cat_id,cat_parent_id, cat_name, cat_image,architect_id) VALUES ('".$cat_id."','".$cat_parent_id."', '".$cat_name."', '".$cat_image."','".$architect_id."');");
...
function delmultirec($id)
{
	...
	$sql = "SELECT cat_id,gallery_id,gallery_image,multi_images
	        FROM tbl_gallery
	...
	$result = dbQuery($sql);
		...
		while ($row = dbFetchAssoc($result)) {
			...
			//echo $row[0];die;
				...
				if($cntCatId>1)
				{
				  $sqlupdate =mysqli_query("UPDATE tbl_gallery SET cat_id = replace(cat_id, '".$categories[$i].",', '') WHERE FIND_IN_SET('".$categories[$i]."',cat_id)>0");

				}
				elseif($cntCatId== 1)
				{
					$image_list = explode(',',$row['gallery_image']);
					...
					foreach($image_list as $row2)
					{
						...
						$new_image_list = '';
						unlink('../gallery/big_img/'.$row2);
						unlink('../gallery/'.$row2);
					}
					
					//pdf file
					//unlink("../gallery_pdf/". $row['pdf_file']);	
					//end of pdf
					...
					foreach($image_list1 as $rowF1)
					{
						//print_r($image_list);die;
						$new_image_list1 = '';
						unlink('../galleryF/big_img/'.$rowF1);
						unlink('../galleryF/'.$rowF1);
					}
					// end of multiple images
					
					// delete the products
					$sqlD = "DELETE FROM tbl_gallery
    ...
    $sql = "DELETE FROM tbl_gallery_category 
            WHERE cat_id IN (" . implode(',', $categories) . ")";
    dbQuery($sql);
...
function deleteImage()
{
	...
	$sql = "UPDATE tbl_gallery_category
			SET cat_image = ''
			WHERE cat_id = $catId";
	dbQuery($sql);        
...
function _deleteImage($id)
{
    ...
    if (dbNumRows($result)) {
        while ($row = dbFetchAssoc($result)) {
			...
			$deleted = @unlink("../gallerycategory/". $row['cat_image']);	
			@unlink("../gallerycategory/big_img/". $row['cat_image']);	
        }	
        $deleted = true;
    }
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Gallery Category View</h4>
                                <?php
                                    if(isset($_POST['delete']))
                                    {
                                        ...
                                        echo "<script>window.open('galleryCatView.php','_SELF')</script>";
                                    }
                                ?>
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row mt-2 mb-2">
                                        <div class="col-lg-12">
                                            <a href="galleryCategory.php?parentid=<?php echo $parentid; ?>" class="btn btn-success m-b-sm">Add Category</a>
								            <a href="sorting_gallery_category.php" class="btn btn-success m-b-sm">Sorting</a>
									...
									<tfoot>
										...
										if (mysqli_num_rows($result) > 0) 
										{
                                            ...
                                            <tr>
												...
												<td><a href="galleryCatView.php?parentid=<?php echo $cat_id; ?>"><?php echo $cat_name; ?></a></td>
												<td><?php if($cat_image!=''){?><img src='../gallerycategory/<?php echo $cat_image; ?>' height="100" width="100"><?php } else { echo "No Image"; }?></td>
                                                <td>
												<a href='galleryCategory_copy.php?id=<?php echo $cat_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a>
												
												</td>
												
												<td><a href='gallery_category_up.php?category_id=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
												<td><a href='deleteGalleryCat.php?catId=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
